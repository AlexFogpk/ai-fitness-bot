...
def is_topic_by_regex(text: str) -> bool:
    # Расширенный список, включающий фитнес, тренировки, здоровье, питание,
    # а также "вредные" продукты, напитки, фастфуд и т.д.
    patterns = [
        # 1. Фитнес, тренировки, здоровье
        ...
        # 2. Питание, продукты, блюда, сладости
        ...
        r"\bпицц\w*", r"\bролл\w*", r"\bсуши\w*", r"\bхотдог\w*", r"\bсэндвич\w*",
        r"\bбутерброд\w*", r"\bджанкфуд\w*", r"\bjunk food\b",

        # 4. Напитки (включая газировку и алкоголь)
        r"\bгазиров\w*", r"\bкол\w*", r"\bпепси\w*", r"\bспрайт\w*", r"\bэнергетик\w*",
        r"\bалкогол\w*", r"\bпиво\w*", r"\bвино\w*", r"\bспиртн\w*"
    ]
...
async def ask_gpt(user_id: str, user_message: str) -> str:
    ...
    # Обновлённый system-промпт для "вредной" еды и т.д.
    system_message = (
        ...
        "Если пользователь спрашивает про здоровое питание, тренировки, баланс, тоже помогай. "
