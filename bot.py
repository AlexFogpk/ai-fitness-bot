...
def is_topic_by_regex(text: str) -> bool:
    # Расширенный список, включающий еду, сладости и т.д.
    patterns = [
        ...
        r"\bгипертроф\w*", r"\bвыносливост\w*", r"\bдинамик\w*", r"\bскорост\w*",
        r"\bфутбол\w*", r"\bбокс\w*", r"\bбег\w*", r"\bвелосипед\w*", r"\bплавани\w*",
        r"\bлыжи\w*", r"\bсоревнован\w*", r"\batлет\w*", r"\bспортсмен\w*",
...
async def is_fitness_question_combined(user_id: str, text: str) -> bool:
    ...
    if is_health_restriction_question(text):
        return True
    # Если совпало по регуляркам (слова про фитнес/тренировки/еду/и т.д.)
...
async def ask_gpt(user_id: str, user_message: str) -> str:
    ...
    system_message = (
        ...
        "Если вопрос о питании (включая любые продукты, десерты и т.д.), давай советы по калориям, балансу, "
